...
encode({on_subscribe, Timestamp, {MP, ClientId, UserName, Topics}}) ->
  encode_envelope("OnSubscribe", on_subscribe_pb:encode_msg(#'eventssidecar.v1.OnSubscribe'{client_id = ClientId,
                                                                            mountpoint = MP,
                                                                            username = UserName,
...
encode({on_unsubscribe, Timestamp, {MP, ClientId, UserName, Topics}}) ->
  encode_envelope("OnUnsubscribe", on_unsubscribe_pb:encode_msg(#'eventssidecar.v1.OnUnsubscribe'{client_id = ClientId,
                                                                                  mountpoint = MP,
                                                                                  username = UserName,
                                                                                  topics = Topics,
                                                                                  timestamp = convert_timestamp(Timestamp)}));
encode({on_deliver, Timestamp, {MP, ClientId, UserName, QoS, Topic, Payload, IsRetain}}) ->
  encode_envelope("OnDeliver", on_deliver_pb:encode_msg(#'eventssidecar.v1.OnDeliver'{client_id = ClientId,
                                                                      mountpoint = MP,
                                                                      username = UserName,
                                                                      topic = Topic,
                                                                      qos = QoS,
...
encode({on_delivery_complete, Timestamp, {MP, ClientId, UserName, QoS, Topic, Payload, IsRetain}}) ->
  encode_envelope("OnDeliveryComplete", on_delivery_complete_pb:encode_msg(#'eventssidecar.v1.OnDeliveryComplete'{client_id = ClientId,
                                                                      mountpoint = MP,
                                                                      username = UserName,
                                                                      topic = Topic,
                                                                      qos = QoS,
...
encode({on_offline_message, Timestamp, {MP, ClientId, QoS, Topic, Payload, IsRetain}}) ->
  encode_envelope("OnOfflineMessage", on_offline_message_pb:encode_msg(#'eventssidecar.v1.OnOfflineMessage'{client_id = ClientId,
                                                                      mountpoint = MP,
                                                                      qos = QoS,
                                                                      topic = Topic,
                                                                      payload = Payload,
...
encode({on_client_wakeup, Timestamp, {MP, ClientId}}) ->
  encode_envelope("OnClientWakeUp", on_client_wakeup_pb:encode_msg(#'eventssidecar.v1.OnClientWakeUp'{client_id = ClientId,
                                                                                       mountpoint = MP,
                                                                                       timestamp = convert_timestamp(Timestamp)}));
encode({on_client_offline, Timestamp, {MP, ClientId}}) ->
  encode_envelope("OnClientOffline", on_client_offline_pb:encode_msg(#'eventssidecar.v1.OnClientOffline'{client_id = ClientId,
                                                                                          mountpoint = MP,
                                                                                          timestamp = convert_timestamp(Timestamp)}));
encode({on_client_gone, Timestamp, {MP, ClientId}}) ->
  encode_envelope("OnClientGone", on_client_gone_pb:encode_msg(#'eventssidecar.v1.OnClientGone'{client_id = ClientId,
                                                                                 mountpoint = MP,
                                                                                 timestamp = convert_timestamp(Timestamp)}));
encode({on_session_expired, Timestamp, {MP, ClientId}}) ->
  encode_envelope("OnSessionExpired", on_session_expired_pb:encode_msg(#'eventssidecar.v1.OnSessionExpired'{client_id = ClientId,
                                                                                          mountpoint = MP,
                                                                                          timestamp = convert_timestamp(Timestamp)}));
encode(_) ->
  <<>>.
