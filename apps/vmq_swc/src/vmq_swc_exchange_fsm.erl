...
-record(state, {config, group, peer, timeout, local_clock, remote_clock, remote_watermark, obj_cnt=0, batch_size, missing_dots}).

start_link(#swc_config{} = Config, Peer, Timeout) ->
    gen_statem:start_link(?MODULE, [Config, Peer, Timeout], []).

% State functions
prepare(internal, start, #state{config=Config, group=Group, peer=Peer, timeout=Timeout} = State) ->
    case vmq_swc_store:lock(Config) of
        ok ->
            %% get remote lock
            lager:debug("Replica ~p: AE exchange with ~p successfully acquired local lock", [Group, Peer]),
        ...
        {error, already_locked} ->
            lager:debug("Replica ~p: AE exchange with ~p terminated due to local store is locked", [Group, Peer]),
            {stop, normal, State};
        Error ->
            lager:error("Replica ~p: AE exchange with ~p can't acquire local lock due to ~p", [Group, Peer, Error]),
            {stop, normal, State}
    end;

prepare(state_timeout, PrepStep, #state{group=Group, peer=Peer} = State) ->
    lager:warning("Replica ~p: AE exchange with ~p prepare step timed out in ~p", [Group, Peer, PrepStep]),
    {stop, normal, State};

prepare(cast, {remote_node_clock, {error, Reason}}, #state{group=Group, peer=Peer} = State) ->
    %% Failed to get remote node clock
    lager:warning("Replica ~p: AE exchange with ~p couldn't request remote node clock due to ~p", [Group, Peer, Reason]),
    teardown(State);

prepare(cast, {remote_node_clock, NodeClock}, #state{group=Group, peer=Peer, config=Config, timeout=Timeout} = State0) ->
    lager:debug("Replica ~p: AE exchange with ~p successfully requested remote node clock", [Group, Peer]),
...
prepare(cast, {remote_watermark, {error, Reason}}, #state{group=Group, peer=Peer} = State) ->
    %% Failed to get remote node clock
    lager:warning("Replica ~p: AE exchange with ~p couldn't request remote watermark due to ~p", [Group, Peer, Reason]),
    teardown(State);

prepare(cast, {remote_watermark, Watermark}, #state{group=Group, peer=Peer} = State0) ->
    lager:debug("Replica ~p: AE exchange with ~p successfully requested remote watermark", [Group, Peer]),
    {next_state, update_local, State0#state{remote_watermark=Watermark},
     [{next_event, internal, start}]}.

update_local(internal, start, #state{config=_Config, peer=_RemotePeer, local_clock=NodeClock, remote_clock=RemoteClock} = State) ->
...
local_sync_repair(cast, {ok, MissingObjects}, #state{config=Config, peer=RemotePeer, remote_clock=RemoteClock, remote_watermark=RemoteWatermark, obj_cnt=ObjCnt} = State) ->
    case State#state.missing_dots of
        [] ->
            vmq_swc_store:sync_repair(Config, MissingObjects, RemotePeer, swc_node:base(RemoteClock), {true, RemoteWatermark}),
            teardown(State#state{obj_cnt=ObjCnt + length(MissingObjects)});
        _ ->
            vmq_swc_store:sync_repair(Config, MissingObjects, RemotePeer, swc_node:base(RemoteClock), false),
...
local_sync_repair(cast, Msg, #state{group=Group, peer=Peer} = State) ->
    lager:error("Replica ~p: AE exchange with ~p received unknown message during local sync repair: ~p", [Group, Peer, Msg]),
    teardown(State);
local_sync_repair(state_timeout, sync_repair, #state{group=Group, peer=Peer} = State) ->
    lager:warning("Replica ~p: AE exchange with ~p couldn't sync repair local store due to timeout", [Group, Peer]),
    teardown(State).
...
terminate(Reason, StateName, #state{group=Group, peer=Peer}) ->
    lager:debug("Replica ~p: AE exchange with ~p terminates with reason ~p in state ~p",
