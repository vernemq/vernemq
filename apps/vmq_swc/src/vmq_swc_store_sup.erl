...
init([#swc_config{group=SwcGroup} = Config, Strategy, Transport]) ->
    ...
    MembershipChildSpec = #{id => {vmq_swc_group_membership, SwcGroup},
                            start => {vmq_swc_group_membership, start_link, [Config, Strategy, Transport]}},
    ...
    {ok, {SupFlags, DBChildSpecs ++ [ExchangeSupChildSpec, MembershipChildSpec, BatcherChildSpec, IteratorChildSpec, StoreChildSpec]}}.

config(PeerName, SwcGroup, TransportMod) when is_binary(PeerName) and is_atom(SwcGroup) and is_atom(TransportMod) ->
