...
handle_info(histogram, #state{histogram=Hist0} = State) ->
    maps:fold(fun(ResType, Data0, _Acc) ->
                      L = length(Data0),
                      ...
                      lager:info("Churney[~p] n: ~p min: ~p max: ~p avg: ~p median: ~p", [ResType, L, Min, Max, Avg, Median])
              end, ignore, Hist0),
    erlang:send_after(10000, self(), histogram),
...
churney_connect(ClientId, {Addr, Port}) ->
    ...
    case packet:do_client_connect(Connect, Connack, [{hostname, Addr},
                                                     {port, Port}]) of
        {ok, Socket} ->
            Socket;
        _E ->
...
ensure_subscription_replicated(Node, ClientId, Topic, N)->
    ...
    case rpc:call(Node, vmq_swc_plugin, metadata_get,
                  [FullPrefix, {"", ClientId}, [{allow_put, false},
                                                {default, this_is_the_default},
                                                {resolver, fun(V) -> V end}]]) of
        this_is_the_default ->
            timer:sleep(10),
