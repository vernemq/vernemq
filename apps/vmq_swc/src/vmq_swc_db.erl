...
delete(#swc_config{db_backend=Backend} = Config, Type, Key) ->
    vmq_swc_metrics:timed_measurement({?METRIC, delete}, Backend, write, [Config, [{Type, Key, ?DELETED}], []]).
...
fold(#swc_config{db_backend=Backend} = Config, Type, FoldFun, Acc, StartKey) ->
    vmq_swc_metrics:timed_measurement({?METRIC, scan}, Backend, fold, [Config, Type, FoldFun, Acc, StartKey]).
