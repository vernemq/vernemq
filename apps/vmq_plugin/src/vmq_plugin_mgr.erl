...
enable_module_plugin(HookName, Module, Fun, Arity, Opts) when
    is_atom(HookName) and is_atom(Module) and
        is_atom(Fun) and (Arity >= 0)
->
    Hook =
        case lists:keyfind(compat, 1, Opts) of
            {compat, {CH, CM, CF, CA}} ->
                #hook{
                    name = HookName,
                    module = Module,
...
handle_deferred_calls(#state{deferred_calls = []} = State) ->
    State.
...
get_new_hooks({application, Name, Opts}, OldPlugins) ->
    HasAppHook = lists:any(
        fun
            ({application, N, _}) -> Name =:= N;
            (_) -> false
        end,
        OldPlugins
    ),
...
plugins_have_hook(
    #hook{name = Name, module = Module, function = Fun, arity = Arity, compat = Compat}, OldPlugins
) ->
    lists:any(
        fun(
            #hook{
...
remove_module_hook(
    #hook{name = Name, module = Module, function = Function, arity = Arity, compat = Compat}, Hooks
) ->
    lists:filter(
        fun
            (#hook{name = N, module = M, function = F, arity = A, compat = C}) ->
...
disable_app_module_plugins(App, State) ->
    ...
    lists:foldl(
        fun({Name, Mod, Fun, Arity}, AccState) ->
...
check_app_hooks(App, [#hook{module = Module, function = Fun, arity = Arity, opts = Opts} | Rest]) when
    is_list(Opts)
->
...
compile_hooks(CheckedPlugins) ->
    ...
    Hooks = lists:sort(
        fun(#hook{name = LName}, #hook{name = RName}) ->
            LName =< RName
        end,
        lists:flatten(RawPlugins)
    ),
...
info_raw_clause(Hooks) ->
    {clause, 1, [{atom, 1, raw}], [], [erl_parse:abstract(Hooks)]}.
info_only_clause(Hooks) ->
    {clause, 1, [{atom, 1, only}], [], [list_const(true, embed_hooks(Hooks))]}.
info_all_clause(Hooks) ->
    {clause, 2, [{atom, 1, all}], [], [list_const(true, embed_hooks(Hooks))]}.
        ...
        case EmbedHook of
            #hook{
                ...
                [
                    {atom, 1, CompatMod},
                    {atom, 1, CompatFun},
                    {cons, 1, {atom, 1, OrigName},
                        {cons, 1, {atom, 1, Module},
                            {cons, 1, {atom, 1, Fun}, {cons, 1, {var, 1, 'Params'}, {nil, 1}}}}}
                ];
        ...
        clause(
            ...
            [
                {call, 1, {atom, 1, apply}, [
                    ...
                    {atom, 1, all},
                    {cons, 1, list_const(false, embed_hooks(EmbedHooks)),
                        {cons, 1, {var, 1, 'Params'}, {nil, 1}}}
                ]}
            ]
        ),
...
check_plugin_for_module_plugin_test() ->
    Plugins = [
        {module, ?MODULE, [
            {hooks, [
                #hook{name = sample_hook, module = ?MODULE, function = sample_hook, arity = 0},
...
vmq_plugin_test() ->
    ...
    ?assertEqual(
        [
            ...
            {sample_hook, vmq_plugin_mgr, sample_hook, 3}
        ],
        vmq_plugin:info(all)
    ),
...
call_hooks() ->
    ...
    ?assertEqual(
        [
            {other_sample_hook_a, 1, 10},
            {other_sample_hook_b, 1, 10},
            {other_sample_hook_c, 1, 10}
        ],
        vmq_plugin:all(sample_all_hook, [10])
    ),
