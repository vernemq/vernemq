...
register_subscriber_(SessionPid, SubscriberId, StartClean, QueueOpts, N, Reason) ->
    ...
    case vmq_queue:info(QPid) of
        ...
        _ ->
            ...
            {SubscriptionsPresent, OldSubs, UpdatedSubs, ChangedNodes, NeedsWrite} =
            ...
            SessionPresent2 =
                case StartClean of
                    true ->
                        ...
                        maybe_commit(NeedsWrite, SubscriberId, OldSubs, UpdatedSubs),
                        false;
                            ...
                            ok ->
                                ...
                                case block_until_drain(SubscriberId, ChangedNodes, BlockCondFun) of
                                    ok ->
                                        ...
                                        maybe_commit(
                                            NeedsWrite, SubscriberId, OldSubs, UpdatedSubs
                                        );
                                    timeout ->
...
%% WITHOUT writing. Returns {SubsPresent, OldSubs, NewSubs, ChangedNodes, NeedsWrite}.
-spec compute_remap_subscriber(subscriber_id(), boolean()) ->
    {boolean(), vmq_subscriber:subs(), vmq_subscriber:subs(), [node()], boolean()}.
compute_remap_subscriber(SubscriberId, _StartClean = true) ->
    OldSubs = subscriptions_for_subscriber_id(SubscriberId),
    Subs = vmq_subscriber:new(true),
    {false, OldSubs, Subs, [], true};
        ...
        Subs ->
            case vmq_subscriber:change_node_all(Subs, node(), false) of
                {NewSubs, ChangedNodes} when length(ChangedNodes) > 0 ->
                    {true, Subs, NewSubs, ChangedNodes, true};
                _ ->
                    {true, Subs, Subs, [], false}
            end
    end.

%% commit_remap_subscriber/3 writes the computed subs to metadata store and
%% updates the local registration view immediately.
-spec commit_remap_subscriber(subscriber_id(), vmq_subscriber:subs(), vmq_subscriber:subs()) -> ok.
commit_remap_subscriber(SubscriberId, OldSubs, Subs) ->
    vmq_subscriber_db:store(SubscriberId, Subs),
    sync_reg_view_update(SubscriberId, OldSubs, Subs).

%% maybe_commit/4 writes metadata only if NeedsWrite is true.
maybe_commit(true, SubscriberId, OldSubs, Subs) ->
    commit_remap_subscriber(SubscriberId, OldSubs, Subs);
maybe_commit(false, _SubscriberId, _OldSubs, _Subs) ->
    ok.
