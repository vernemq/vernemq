...
start_link(SubscriberId, Clean) ->
    gen_statem:start_link(?MODULE, [SubscriberId, Clean], []).


active(Queue) when is_pid(Queue) ->
    gen_statem:send_event(Queue, {change_state, active, self()}).

notify(Queue) when is_pid(Queue) ->
    gen_statem:send_event(Queue, {change_state, notify, self()}).

notify_recv(Queue) when is_pid(Queue) ->
    gen_statem:send_event(Queue, {notify_recv, self()}).

enqueue(Queue, Msg) when is_pid(Queue) ->
    gen_statem:send_event(Queue, {enqueue, to_internal(Msg)}).

enqueue_many(Queue, Msgs) when is_pid(Queue) and is_list(Msgs) ->
    NMsgs = lists:map(fun to_internal/1, Msgs),
    gen_statem:sync_send_event(Queue, {enqueue_many, NMsgs}, infinity).

enqueue_many(Queue, Msgs, Opts) when is_pid(Queue), is_list(Msgs), is_map(Opts) ->
    NMsgs = lists:map(fun to_internal/1, Msgs),
    gen_statem:sync_send_event(Queue, {enqueue_many, NMsgs, Opts}, infinity).

-spec add_session(pid(), pid(), map()) -> {ok, #{initial_msg_id := msg_id()}} |
                                          {error, any()}.
add_session(Queue, SessionPid, Opts) when is_pid(Queue) ->
    gen_statem:sync_send_event(Queue, {add_session, SessionPid, Opts}, infinity).

get_sessions(Queue) when is_pid(Queue) ->
    gen_statem:sync_send_all_state_event(Queue, get_sessions, infinity).

set_opts(Queue, Opts) when is_pid(Queue) ->
    gen_statem:sync_send_event(Queue, {set_opts, self(), Opts}, infinity).

get_opts(Queue) when is_pid(Queue) ->
    case catch gen_statem:sync_send_all_state_event(Queue, get_opts, infinity) of
        {'EXIT', {normal, _}} ->
            {error, noproc};
        {'EXIT', {noproc, _}} ->
            {error, noproc};
...
force_disconnect(Queue, Reason, DoCleanup) when is_pid(Queue) and is_boolean(DoCleanup) ->
    gen_statem:sync_send_all_state_event(Queue, {force_disconnect, Reason, DoCleanup}, infinity).

set_delayed_will(Queue, Fun, Delay) when is_pid(Queue) ->
    gen_statem:sync_send_all_state_event(Queue, {set_delayed_will, Fun, Delay}, infinity).

set_last_waiting_acks(Queue, WAcks, NextMsgId) ->
    gen_statem:sync_send_event(Queue, {set_last_waiting_acks, WAcks, NextMsgId}, infinity).
...
save_sync_send_event(Queue, Event) ->
    case catch gen_statem:sync_send_event(Queue, Event, infinity) of
        {'EXIT', {normal, _}} ->
            ok;
        {'EXIT', {noproc, _}} ->
            ok;
...
status(Queue) ->
    gen_statem:sync_send_all_state_event(Queue, status, infinity).

info(Queue) ->
    gen_statem:sync_send_all_state_event(Queue, info, infinity).
...
wait_for_offline({add_session, SessionPid, Opts}, From,
    ...
    gen_statem:reply(MigrationFrom, ok),
...
wait_for_offline({add_session, NewSessionPid, NewOpts}, From,
    ...
    gen_statem:reply(AddFrom, {ok, Opts}),
...
drain(drain_start, #state{id=SId, offline=#queue{queue=Q} = Queue,
    ...
    case queue:to_list(DecompressedDrainQ) of
        [] ->
            ...
            {next_state, drain,
             State#state{drain_over_timer=gen_statem:send_event_after(DrainTimeout, drain_over),
...
offline(init_offline_queue, #state{id=SId} = State) ->
    case vmq_plugin:only(msg_store_find, [SId]) of
        ...
        {error, Reason} ->
            ...
            gen_statem:send_event_after(1000, init_offline_queue),
...
init([SubscriberId, Clean]) ->
    ...
    case Clean of
        ...
        false ->
            ...
            gen_statem:send_event(self(), init_offline_queue)
    end,
...
handle_session_down(SessionPid, StateName,
    ...
    case {maps:size(NewState#state.sessions), StateName, WaitingCall} of
        {0, wait_for_offline, {add_session, NewSessionPid, Opts, From}} ->
            ...
            gen_statem:reply(From, {ok, RetOpts}),
...
maybe_set_expiry_timer(ExpireAfter, State) when ExpireAfter > 0 ->
    Ref = erlang:send_after(ExpireAfter * 1000, expire_session),
...
maybe_set_last_will_timer(#state{delayed_will = {Delay, _Fun}} = State) ->
    Ref = erlang:send_after(Delay * 1000, publish_last_will),
