...
get_label_info() ->
    LabelInfo =
        lists:foldl(
            ...
            #{},
            metric_defs() ++ pluggable_metric_defs()
        ),
...
misc_statistics() ->
    ...
    {BalanceAccepting, BalanceLocalConns, BalanceClusterAvgF, BalanceEnabled} =
        fetch_external_metric(vmq_balance_srv, balance_stats, {1, 0, 0.0, 0}),
    BalanceRpcFailures = fetch_external_metric(vmq_balance_srv, rpc_failures_total, 0),
    %% Prometheus serializer expects integer values; round here so the metric
    %% surface stays integer while the HTTP body keeps full precision.
    BalanceClusterAvg = round(BalanceClusterAvgF),
    [
        ...
        {balance_rpc_failures_total, BalanceRpcFailures}
    ].
...
misc_stats_def() ->
    [
        ...
        m(
            counter,
            [],
            balance_rpc_failures_total,
            balance_rpc_failures_total,
            <<
                "Cumulative count of failed peer count-collection RPCs since "
                "node start. Includes both badrpc replies and unreachable nodes."
            >>
        )
    ].
