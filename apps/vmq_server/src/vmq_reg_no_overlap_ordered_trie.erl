...
-module(vmq_reg_no_overlap_ordered_trie).
...
fold({MP, _} = SubscriberId, Topic, FoldFun, Acc) when is_list(Topic) ->
    ...
    Subs =
        maps:values(apply_sort(group_subs(acc_all_subs(SubscriberId, MatchedTopics)))),
...
fold_(
    {_MP, _} = SubscriberId, FoldFun, Acc, [{{_Topic, _SubsIdQoS} = Sub, Node} | Subs], Remotes
) when
    Node == node()
->
    fold_(
...
fold_(SubscriberId, FoldFun, Acc, [{_Sub, Node} | Subs], Remotes) ->
    case lists:member(Node, Remotes) of
        true ->
            fold_(SubscriberId, FoldFun, Acc, Subs, Remotes);
        false ->
            fold_(SubscriberId, FoldFun, FoldFun(Node, SubscriberId, Acc), Subs, [
                Node | Remotes
            ])
    end;
fold_(_, _, Acc, [], _) ->
...
lookup_subs(Key) ->
    case ets:lookup(vmq_ordered_tree_subs, Key) of
        [{_, fanout}] ->
            MS = [{{{Key, '$1'}}, [], [{{{Key}, '$1'}}]}],
            ets:select(vmq_ordered_subs_fanout, MS);
        Res ->
            Res
    end.

acc_all_subs({MP, _} = _SubscriberId, MatchedTopics) ->
    F =
        fun
            % shared sub
            ({Topic, {Node, Group}}) ->
                S = lookup_subs({MP, Group, Node, Topic}),
                add_node(Node, S);
            ({Topic, Node}) ->
                S = lookup_subs({MP, Topic}),
                add_node(Node, S)
        end,
    lists:map(F, MatchedTopics).
...
group_subs(Subs) ->
    maps:groups_from_list(
        fun
            ({{_, {{_, ClientId}, _}}, _}) ->
                ClientId;
            ({{_, {_, _, {_, ClientId}, _}}, _}) ->
                % We need to separate shared subscribers from normal subscribers,
                % but the same MQTT client can be both. We do separate overlap sorting
                % for both routing decisions.
                {shared, ClientId}
        end,
        lists:flatten(Subs)
    ).
    ...
    lists:sort(
        fun
            (
                ...
                % implicit structure of subscription
                QoSA > QoSB;
            (
                {{_, {_, _, _, QosA}}, _},
                {{_, {_, _, _, QosB}}, _}
...
apply_sort(Map) ->
    ...
    #{Client => lists:nth(1, sort_subs(V)) || Client := V <- Map}.
...
stats() ->
    NrOfSubs = info(vmq_ordered_tree_subs, size),
    NrOfRemoteSubs = info(vmq_ordered_trie_remote_subs, size),
    Mem1 = info(vmq_ordered_tree_subs, memory),
    Mem2 = info(vmq_ordered_trie_topic, memory),
    Mem3 = info(vmq_ordered_trie, memory),
    Mem4 = info(vmq_ordered_trie_node, memory),
    Mem5 = info(vmq_ordered_trie_remote_subs, memory),
    Mem6 = info(vmq_ordered_subs_fanout, memory),
...
init([]) ->
    ...
    _ = ets:new(vmq_ordered_trie, [{keypos, 2} | DefaultETSOpts]),
    _ = ets:new(vmq_ordered_trie_node, [{keypos, 2} | DefaultETSOpts]),
    _ = ets:new(vmq_ordered_trie_topic, [{keypos, 1} | DefaultETSOpts]),
    _ = ets:new(vmq_ordered_tree_subs, [bag | DefaultETSOpts]),
    _ = ets:new(vmq_ordered_subs_fanout, [ordered_set | DefaultETSOpts]),
    _ = ets:new(vmq_ordered_trie_remote_subs, [{keypos, 1} | DefaultETSOpts]),
    Self = self(),
    spawn_link(
        fun() ->
...
handle_event(Handler, Event) ->
    case Handler(Event) of
        ...
        {update, SubscriberId, OldValue, NewValue} ->
            {ToRemove, ToAdd} = vmq_subscriber:get_changes(one_pass_ordered, OldValue, NewValue),
...
match(MP, Topic, [#trie_node{topic = Name} | Rest], Acc) when Name =/= undefined ->
    case ets:lookup(vmq_ordered_trie_topic, {MP, Name}) of
        [] ->
            match(MP, Topic, Rest, Acc);
        [{_, _, Nodes}] ->
...
add_complex_topic(MP, Topic, Node, true) ->
    MPTopic = {MP, Topic},
    case ets:lookup(vmq_ordered_trie_topic, MPTopic) of
        [] ->
            ets:insert(vmq_ordered_trie_topic, {MPTopic, 1, [{Node, 1}]});
        [{_, TotalCnt, Nodes}] ->
            NewNodes = add_and_inc(Node, Nodes),
            ets:insert(vmq_ordered_trie_topic, {MPTopic, TotalCnt + 1, NewNodes})
    end,

    case ets:lookup(vmq_ordered_trie_node, MPTopic) of
        ...
        _ ->
            ...
            %% add last node
            ets:insert(vmq_ordered_trie_node, #trie_node{node_id = MPTopic, topic = Topic})
    end.

trie_add_path(MP, {Node, Word, Child}) ->
    NodeId = {MP, Node},
    Edge = #trie_edge{node_id = NodeId, word = Word},
    case ets:lookup(vmq_ordered_trie_node, NodeId) of
        [TrieNode = #trie_node{edge_count = Count}] ->
            case ets:lookup(vmq_ordered_trie, Edge) of
                [] ->
                    ets:insert(
                        vmq_ordered_trie_node,
                        TrieNode#trie_node{edge_count = Count + 1}
                    ),
                    ets:insert(vmq_ordered_trie, #trie{edge = Edge, node_id = Child});
                [_] ->
                    ok
            end;
        [] ->
            ets:insert(vmq_ordered_trie_node, #trie_node{node_id = NodeId, edge_count = 1}),
            ets:insert(vmq_ordered_trie, #trie{edge = Edge, node_id = Child})
    end.
...
trie_match(MP, Node, [], ResAcc) ->
    NodeId = {MP, Node},
    ets:lookup(vmq_ordered_trie_node, NodeId) ++ 'trie_match_#'(NodeId, ResAcc);
...
'trie_match_#'({MP, _} = NodeId, ResAcc) ->
    case ets:lookup(vmq_ordered_trie, #trie_edge{node_id = NodeId, word = <<"#">>}) of
        [#trie{node_id = ChildId}] ->
            ets:lookup(vmq_ordered_trie_node, {MP, ChildId}) ++ ResAcc;
        [] ->
            ResAcc
    end.
...
del_complex_topic(MP, Topic, NodeOrGroup, true) ->
    ...
    case ets:lookup(vmq_ordered_trie_topic, MPTopic) of
        [{_, TotalCnt, Nodes}] when TotalCnt > 1 ->
            ...
            ets:insert(vmq_ordered_trie_topic, {MPTopic, TotalCnt - 1, NewNodes});
        [{_, 1, _}] ->
            ets:delete(vmq_ordered_trie_topic, MPTopic),
...
trie_delete(MP, Topic) ->
    ...
    case ets:lookup(vmq_ordered_trie_node, NodeId) of
        [#trie_node{edge_count = 0}] ->
            ets:delete(vmq_ordered_trie_node, NodeId),
...
trie_delete_path(MP, [{Node, Word, _} | RestPath]) ->
    NodeId = {MP, Node},
    Edge = #trie_edge{node_id = NodeId, word = Word},
    ets:delete(vmq_ordered_trie, Edge),
    case ets:lookup(vmq_ordered_trie_node, NodeId) of
        [#trie_node{edge_count = 1, topic = undefined}] ->
            ets:delete(vmq_ordered_trie_node, NodeId),
            trie_delete_path(MP, RestPath);
        [#trie_node{edge_count = Count} = TrieNode] ->
            ets:insert(vmq_ordered_trie_node, TrieNode#trie_node{edge_count = Count - 1});
        [] ->
            ignore
    end.
...
insert_trie_subs(Key, Val) ->
    ...
    case ets:lookup(vmq_ordered_tree_subs, Key) of
        [] ->
            ets:insert(vmq_ordered_tree_subs, E);
        [E] ->
            %% duplicate - do nothing;
            true;
        [{Key, fanout}] ->
            ets:insert(vmq_ordered_subs_fanout, {E});
        [E1] ->
            %% fanout - move to fanout table
            ets:delete(vmq_ordered_tree_subs, Key),
            ets:insert(vmq_ordered_tree_subs, {Key, fanout}),
            ets:insert(vmq_ordered_subs_fanout, {E}),
            ets:insert(vmq_ordered_subs_fanout, {E1})
    end.
...
del_trie_subs(Key, Val) ->
    case ets:lookup(vmq_ordered_tree_subs, Key) of
        ...
        [{Key, fanout}] ->
            ...
            ets:delete(vmq_ordered_subs_fanout, {Key, Val}),

            %% select to retrieve max 2 results to determine if we
            %% need to move back to the normal table.
            MS = [{{{Key, '$1'}}, [], [{{{Key}, '$1'}}]}],
            case ets:select(vmq_ordered_subs_fanout, MS, 2) of
                {[E], _Continuation} ->
                    %% last element in the fanout, move to normal table
                    ets:delete(vmq_ordered_subs_fanout, E),
                    ets:delete_object(vmq_ordered_tree_subs, {Key, fanout}),
                    ets:insert(vmq_ordered_tree_subs, E);
                {[_, _], _Continuation} ->
...
add_remote_subscriber(MP, Topic, Node) ->
    Key = {MP, Topic},
    NewRemotes =
        case ets:lookup(vmq_ordered_trie_remote_subs, Key) of
            [] ->
                [{Node, 1}];
            [{_, Remotes}] ->
                add_and_inc(Node, Remotes)
        end,
    ets:insert(vmq_ordered_trie_remote_subs, {Key, NewRemotes}).
    ...
    case ets:lookup(vmq_ordered_trie_remote_subs, Key) of
        ...
        [{_, Remotes}] ->
            case rem_and_dec(Node, Remotes) of
                [] ->
                    ets:delete(vmq_ordered_trie_remote_subs, Key);
                NewRemotes ->
                    ets:insert(vmq_ordered_trie_remote_subs, {Key, NewRemotes})
            end
    end.
