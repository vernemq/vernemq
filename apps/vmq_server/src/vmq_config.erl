...
-export([
    start_link/0,
    change_config/1,
    configure_node/0, configure_node/1,
    configure_nodes/0,
    get_env/1, get_env/2, get_env/3,
    set_env/3, set_env/4, set_env/5,
    set_global_env/4,
    get_all_env/1,
    get_prefixed_env/2,
...
-record(vmq_config,
    % {node(), app_name(), item_name()}
    {key, val, short_descr, long_descr}
).
...
-spec get_env(App, Key, Default) -> Val when
    App :: any(),
    Key :: any(),
    Val :: any(),
    Default :: any().
...
-spec filter_out_unchanged([{Key, Val}], [any()]) -> [{Key, Val}] when
    Key :: any(),
    Val :: any().
