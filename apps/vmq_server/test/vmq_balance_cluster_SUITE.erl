...
balance_skewed_node_rejects_test(Config) ->
    ...
    %% Avg should be NumClients / 3 nodes = 10.0
    ?assert(abs(OvClusterAvg - (NumClients / 3)) < 0.01),
    ...
    application:ensure_all_started(inets),
    HttpPort = vmq_test_utils:get_free_port(),
    ok = rpc:call(OverloadedNode, application, set_env,
                  [vmq_server, http_modules_auth,
                   #{vmq_balance_http => "noauth"}]),
