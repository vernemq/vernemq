...
balance_srv_stats_returns_tuple_test(_Config) ->
    ...
    ?assert(LocalConns >= 0),
    ?assert(is_float(ClusterAvg)),
    ?assert(ClusterAvg >= 0.0),
...
balance_http_json_format_test(_Config) ->
    ...
    ?assert(maps:is_key(<<"enabled">>, Json)),
    %% When disabled/accepting, status should be "accepting" and enabled false
    ?assertEqual(<<"accepting">>, maps:get(<<"status">>, Json)),
    ?assertEqual(false, maps:get(<<"enabled">>, Json)),
...
start_http_listener() ->
    ...
    application:set_env(vmq_server, http_modules_auth, #{vmq_balance_http => "noauth"}),
...
start_metrics_listener() ->
    Port = vmq_test_utils:get_free_port(),
    application:set_env(vmq_server, http_modules_auth,
                        #{vmq_metrics_http => "noauth",
                          vmq_balance_http => "noauth"}),
    vmq_server_cmd:listener_start(Port, [{http, true},
