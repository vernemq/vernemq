...
queue_crash_test(_) ->
    ...
    {ok, #{session_present := false,
           queue_pid := QPid1}} = vmq_reg:register_subscriber(SessionPid1, SubscriberId, false, QueueOpts, 10),
    ...
    {ok, #{session_present := true,
           queue_pid := NewQPid}} = vmq_reg:register_subscriber(SessionPid2, SubscriberId, false, QueueOpts, 10),
...
queue_fifo_test(_) ->
    ...
    {ok, #{session_present := false,
           queue_pid := QPid}} = vmq_reg:register_subscriber(SessionPid1, SubscriberId, false, QueueOpts, 10),
    {ok, [1]} = vmq_reg:subscribe(false, SubscriberId,
                           [{[<<"test">>, <<"fifo">>, <<"topic">>], 1}]),
    %% teardown session
    SessionPid1 ! go_down,
    ...
    SessionPid2 = spawn(fun() -> mock_session(Parent) end),
    {ok, #{session_present := true,
           queue_pid := QPid}} = vmq_reg:register_subscriber(SessionPid2, SubscriberId, false, QueueOpts, 10),
...
queue_force_disconnect_test(_) ->
    ...
    SessionPid1 = spawn(fun() -> mock_session(Parent) end),
    {ok, #{session_present := false,
           queue_pid := QPid0}} = vmq_reg:register_subscriber(SessionPid1, SubscriberId, false, QueueOpts, 10),
    ...
    {ok, #{session_present := true,
           queue_pid := QPid0}} = vmq_reg:register_subscriber(Parent, SubscriberId, false, QueueOpts, 10).
...
queue_force_disconnect_cleanup_test(_) ->
    ...
    {ok, #{session_present := SessionPresent,
           queue_pid := QPid0}} = vmq_reg:register_subscriber(NonConsumingSessionPid, SubscriberId, false, QueueOpts, 10),
    ...
    {ok, #{session_present := SessionPresent,
           queue_pid := QPid1}} = vmq_reg:register_subscriber(NonConsumingSessionPid, SubscriberId, false, QueueOpts, 10),
    true = (QPid0 =/= QPid1),
