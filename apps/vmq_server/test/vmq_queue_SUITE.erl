...
queue_crash_test(_) ->
    ...
    {ok, []} = vmq_lvldb_store:msg_store_find(SubscriberId, other).
...
queue_persistent_client_expiration_test(_) ->
    ...
    timer:sleep(50), % give some time to plumtree
    {ok, FoundMsgs} = vmq_lvldb_store:msg_store_find(SubscriberId, other),
...
enable_hooks() ->
    ...
    vmq_plugin_mgr:enable_module_plugin(vmq_lvldb_store, msg_store_find, 2).
