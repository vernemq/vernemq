...
openssl ca -batch -config openssl.cnf -name CA_signing -extensions server_cert_localhost -out server.crt -infiles server.csr
openssl rsa -aes256 -in server.key -out server.encrypted.key -passout pass:VerneMQ123
...
openssl ca -batch -config openssl.cnf -name CA_signing -extensions server_cert_notlocalhost -out bad-cn-server.crt -infiles bad-cn-server.csr
...
openssl ca -batch -config openssl.cnf -name CA_signing -extensions server_cert_localhost -days 1 -startdate 120820000000Z -enddate 120821000000Z -out server-expired.crt -infiles server-expired.csr
