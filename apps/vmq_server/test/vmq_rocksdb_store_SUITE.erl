-module(vmq_rocksdb_store_SUITE).
...
insert_delete_test(Config) ->
    ...
    {ok, Refs} = vmq_rocksdb_store:msg_store_find({"", "foo"}),
    %% delete all
    ok = delete_msgs({"", "foo"}, Msgs),
    {ok, []} = vmq_rocksdb_store:msg_store_find({"", "foo"}),
...
message_compat_pre_test(_Cfg) ->
    ...
    PreVersion = {[<<"routing">>, <<"key">>], %% routing_key = [<<"routing">>, <<"key">>]
                   <<"payload">>},
    PreVersion = vmq_rocksdb_store:parse_p_msg_val_pre(
                   vmq_rocksdb_store:serialize_p_msg_val_pre(PreVersion)),
    ...
    PreVersion = vmq_rocksdb_store:parse_p_msg_val_pre(
                   vmq_rocksdb_store:serialize_p_msg_val_pre(FutureVersion)),
    ok.
...
idx_compat_pre_test(_Cfg) ->
    ...
    PreVersion = vmq_rocksdb_store:parse_p_idx_val_pre(
                   vmq_rocksdb_store:serialize_p_idx_val_pre(PreVersion)),
    ...
    PreVersion = vmq_rocksdb_store:parse_p_idx_val_pre(
                   vmq_rocksdb_store:serialize_p_idx_val_pre(FutureVersion)),
    ok.
...
store_msgs(SId, [Msg|Rest]) ->
    ok = vmq_rocksdb_store:msg_store_write(SId, Msg),
    store_msgs(SId, Rest);
store_msgs(_, []) -> ok.

delete_msgs(_, []) -> ok;
delete_msgs(SId, [#vmq_msg{msg_ref=Ref}|Rest]) ->
    ok = vmq_rocksdb_store:msg_store_delete(SId, Ref),
...
read_msgs(SId, [Ref|Refs], Acc) ->
    {ok, Msg} = vmq_rocksdb_store:msg_store_read(SId, Ref),
    read_msgs(SId, Refs, [Msg|Acc]).

refcount([Ref|Refs], Cnt) ->
    refcount(Refs, Cnt + vmq_rocksdb_store:refcount(Ref));
refcount([], Cnt) -> Cnt.
...
store_summary() ->
    vmq_rocksdb_store_utils:full_table_scan(
      fun
