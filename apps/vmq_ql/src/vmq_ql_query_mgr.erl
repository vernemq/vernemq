...
handle_info(
    ...
    NewState = fetch_if_ready(State#state{
        waiting = lists:delete({Node, JobPid}, Waiting),
        errors = [Reason | Errors]
    }),
    {noreply, NewState};
handle_info(
    {results_ready, Node, JobPid, _NumResults},
...
%% no query worker alive
results_ready(#state{waiting = [], ready = []}) -> true;
