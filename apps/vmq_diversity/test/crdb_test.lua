...
-- Drop test table
assert(postgres.execute("crdb_test", "DROP TABLE IF EXISTS crdb_test_lua_tbl"))

-- Create test table
assert(postgres.execute("crdb_test", 
    [[CREATE TABLE crdb_test_lua_tbl(
...
-- insert some data
assert(postgres.execute("crdb_test", 
    [[INSERT INTO crdb_test_lua_tbl VALUES
...
-- select
results = postgres.execute("crdb_test", "SELECT * FROM crdb_test_lua_tbl")
...
--assert(postgres.execute("crdb_test", "PREPARE select_all AS SELECT * FROM crdb_test_lua_tbl"))
--
--results = postgres.execute("crdb_test", "EXECUTE select_all")
--assert(#results == 4, "error in select")
--assert_result(results)


-- more complex query
results = postgres.execute("crdb_test", 
