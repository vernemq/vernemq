-- Copyright 2024- Octavo Labs/VerneMQ
...
-- ODBC Configuration, read the documentation below to properly
...
function auth_on_register(reg)
    if reg.mountpoint == 'd' then
        return 'next'
    elseif reg.mountpoint == '' then
        mountpoint = 'd' -- short for default mountpoint ("" or []).
                         -- We do this because Erlang ODBC does not
                         -- support param querys with NULL params, therefore we
                         -- store the 'default' mountpoint as 'd' instead of NULL.
    else
        mountpoint = reg.mountpoint
    end
    if reg.username ~= nil and reg.password ~= nil then
        pwd = obf.decrypt(reg.password)
        -- server_hash = "crypt(?, password)"
        local results = odbc.execute(pool,
            [[SELECT publish_acl, subscribe_acl
              ...
              WHERE
                ...
                username=? AND
                password=]] .. odbc.hash_method(), mountpoint, reg.client_id, reg.username, pwd)
