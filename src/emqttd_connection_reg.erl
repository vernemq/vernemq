...
unsubscribe(Topic) when is_list(Topic) ->
    Client = self(),
	gen_server:cast(?MODULE, {unsubscribe, Topic, Client}).


%publish to cluster node.
publish(MessageId, Topic, Payload) when is_list(Topic) and is_binary(Payload) ->
	lists:foreach(fun(#topic{name=Name, node=Node}) ->
		case Node == node() of
		true -> route(MessageId, Name, Payload);
		false -> rpc:call(Node, ?MODULE, route, [MessageId, Name, Payload])
		end
	end, match(Topic)).

%route locally, should only be called by publish
route(MessageId, Topic, Payload) ->
	[Client ! {route, MessageId, Topic, Payload, Qos} || #subscriber{qos=Qos, client=Client} <- ets:lookup(subscriber, Topic)].
...
init([]) ->
	...
	io:format("emqttd_connection_reg is started~n."),
	{ok, #state{}}.
